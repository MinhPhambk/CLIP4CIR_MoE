...
# Giả định các module này nằm cùng thư mục hoặc trong Python path của bạn
...
def compute_fiq_metrics(relative_dataset: FashionIQDataset, clip_model: CLIP, index_features: torch.tensor,
    ...
    # Lấy top 50 để tính metrics hoặc lưu submission
...
def fashioniq_retrieval(split: str, dress_type: str, combining_function: callable, clip_model: CLIP, preprocess: callable):
    ...
    classic_dataset = FashionIQDataset(split, [dress_type], 'classic', preprocess)
    index_features, index_names = extract_index_features(classic_dataset, clip_model)
    
    relative_dataset = FashionIQDataset(split, [dress_type], 'relative', preprocess)

    return compute_fiq_metrics(relative_dataset, clip_model, index_features, index_names,
                               combining_function, dress_type)

# --- Giữ nguyên các hàm cho CIRR nếu không cần dùng ---
# (Đã lược bỏ phần CIRR để tập trung vào FashionIQ theo yêu cầu, nhưng bạn có thể giữ lại từ code gốc)
